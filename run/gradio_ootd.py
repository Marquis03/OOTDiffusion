...
with block:
    with gr.Row():
        gr.Markdown("# OOTDiffusion 虚拟试衣系统")
    with gr.Row():
        gr.Markdown("## 半身")
    with gr.Row():
        gr.Markdown("***支持上半身的服装***")
    with gr.Row():
        with gr.Column():
            vton_img = gr.Image(
                label="人物",
    ...
    with gr.Column():
        ...
        image_scale = gr.Slider(
            label="Guidance scale", minimum=1.0, maximum=5.0, value=2.0, step=0.1
        )
    ...
    with gr.Column():
        run_button_dc = gr.Button(value="生成")
